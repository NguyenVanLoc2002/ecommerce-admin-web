...
import type { UpdateCarrierConfigRequest } from '../types/carrier.types';

export function useUpdateCarrierConfig() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: UpdateCarrierConfigRequest }) =>
      carrierService.updateConfig(id, body),
    onSuccess: (carrier) => {
